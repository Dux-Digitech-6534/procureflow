...
async function get_parent_mr_from_mr_item(material_request_item_name) {
    ...
    try {
        let r = await frappe.call({
            method: "procureflow.api.get_material_request_from_item",
            args: {
                material_request_item: material_request_item_name
            }
        });
...
function is_po_against_material_request(frm) {
    return (frm.doc.items || []).some(row => {
        return !!(row.material_request || row.material_request_item);
    });
}

function set_purchase_order_mr_fields_readonly(frm) {
    if (!frm || frm.doctype !== "Purchase Order") {
        return;
    }

    const is_from_mr = is_po_against_material_request(frm);
    const parent_fields = [
        "custom_project_name",
    ...
    parent_fields.forEach(fieldname => {
        if (frm.fields_dict[fieldname]) {
            frm.set_df_property(fieldname, "read_only", is_from_mr ? 1 : 0);
            frm.refresh_field(fieldname);
        }
    });
}

function copy_mr_item_remark_to_po_items(frm) {
    if (!frm || frm.doctype !== "Purchase Order") {
        return;
    }

    const rows = (frm.doc.items || []).filter(row => {
        return row.material_request_item && !row.custom_remark;
    });

    rows.forEach(row => {
        frappe.call({
            method: "procureflow.api.get_material_request_item_remark",
            args: {
            ...
            if (value && !row.custom_remark) {
                frappe.model.set_value(row.doctype, row.name, "custom_remark", value);
            }
        });
    });
}

function apply_po_mr_readonly_and_remark_mapping(frm) {
    copy_mr_item_remark_to_po_items(frm);
    set_purchase_order_mr_fields_readonly(frm);
}

frappe.ui.form.on("Purchase Order", {
    refresh: function (frm) {
        apply_po_mr_readonly_and_remark_mapping(frm);
        setTimeout(() => apply_po_mr_readonly_and_remark_mapping(frm), 500);
        setTimeout(() => apply_po_mr_readonly_and_remark_mapping(frm), 1200);
    },
...
frappe.ui.form.on("Purchase Order Item", {
    ...
    material_request: function (frm, cdt, cdn) {
        setTimeout(() => apply_po_mr_readonly_and_remark_mapping(frm), 300);
    },
...
// purchase order company cascadding 
