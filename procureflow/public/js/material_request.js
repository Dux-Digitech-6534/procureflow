...
function open_item_popup(frm) {
    ...
    let d = new frappe.ui.Dialog({
        ...
        primary_action(values) {
            ...
            child.custom_remark = values.custom_remark;
...
function show_pending_po_in_material_request(frm) {
    ...
    const update_label = () => {
        ...
        if (frm.page && frm.page.set_indicator) {
            frm.page.set_indicator(__("Pending PO"), "orange");
        }
        ...
        const selectors = [
            ...
            ".indicator",
            ".badge",
            ".form-dashboard .indicator-pill"
        ];

        selectors.forEach((selector) => {
            frm.page.wrapper.find(selector).each(function () {
                const $el = $(this);
                if ($el.text().trim() === "Pending") {
    ...
    setTimeout(update_label, 200);
    setTimeout(update_label, 600);
    setTimeout(update_label, 1000);
}


frappe.ui.form.on("Material Request", {
    refresh: function (frm) {
        show_pending_po_in_material_request(frm);
