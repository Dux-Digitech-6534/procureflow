...
export const REPORT_DEFS: ReportDef[] = [
	...
	},
	{
		...
		desc: 'Cash-out log — every payment against a receipt or a PO advance.',
		filters: ['date', 'project', 'supplier', 'search'],
		columns: [
			...
			{ key: 'supplier', label: 'Supplier' },
			{ key: 'project', label: 'Project' },
			{ key: 'reference', label: 'Against', type: 'link', link: (r) => r.purchase_receipt ? `/receipts/${r.purchase_receipt}` : `/purchase-orders/${r.purchase_order}` },
			{ key: 'amount', label: 'Amount', type: 'money', align: 'right' },
		],
	},
	{
		slug: 'payment-worklist', title: 'Outstanding Supplier Payments', icon: 'banknote',
		desc: 'Per PO — payments still owed to suppliers, oldest first, with outstanding against the order and against receipts (netting advances and receipt payments) plus payment progress.',
		filters: ['date', 'project', 'supplier'],
		columns: [
			{ key: 'purchase_order', label: 'PO No.', type: 'link', link: (r) => `/purchase-orders/${r.purchase_order}` },
			{ key: 'transaction_date', label: 'PO Date', type: 'date' },
			{ key: 'supplier', label: 'Supplier' },
			{ key: 'project', label: 'Project' },
			{ key: 'po_value', label: 'PO Value', type: 'money', align: 'right' },
			{ key: 'received_value', label: 'Received', type: 'money', align: 'right' },
			{ key: 'paid_amount', label: 'Paid', type: 'money', align: 'right' },
			{ key: 'po_outstanding', label: 'Outstanding vs PO', type: 'money', align: 'right' },
			{ key: 'pr_outstanding', label: 'Outstanding vs PR', type: 'money', align: 'right' },
	...
	},
	{
		...
		desc: 'Per PO — outstanding against the order and against receipts, netting advances and receipt payments. Aged by PO date into 0-30 / 31-60 / 61-90 / 90+ buckets.',
		filters: ['date', 'project', 'supplier'],
		bucketFilter: {
			default: 'all',
			options: [
